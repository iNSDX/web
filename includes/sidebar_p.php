<div id="sidebar">
  <div class="inner">


    <!-- Menu -->
      <nav id="menu">
        <header class="major">
          <h2>Menú</h2>
        </header>
        <ul>
          <li><a href="index.php">Inicio</a></li>
          <li>
											<span class="opener"> Ventas </span>

                      <ul>
                        <li><a href="nVenta.php">Nueva Venta</a></li>
                        <li><a href="hVentas.php">Historial de Ventas</a></li>

											</ul>
          </li>


          <li><a href="facturas.php">Facturas</a></li>
          <li><a href="arqueo.php">Arqueo Caja</a></li>
          <li><a href="contabilidad.php">Contabilidad</a></li>
          <li><a href="informes.php">Informes</a></li>
          <li>
											<span class="opener"> Inventario </span>

                      <ul>
												<li><a href="inventario.php">Stock</a></li>
                        <li><a href="maquinas.php">Máquinas</a></li>
											</ul>

          </li>

          <li><a href="o_Articulo.php">Op. Artículo</a></li>
          <li>
											<span class="opener"> Pedido </span>

                      <ul>
												<li><a href="albaran.php">Albarán</a></li>
                        <li><a href="pedidos.php">Hacer Pedido</a></li>
											</ul>
          </li>




          <li><a href="logout.php">Cerrar Sesión</a></li>
        </ul>
      </nav>



    <!-- Footer -->
      <footer id="footer">
        <p class="copyright">&copy;2017 Estanco de Ibros. Todos los derechos reservados.</p>
      </footer>

  </div>
</div>
